...
/// Manages the Ki chat conversation — history loading and SSE streaming.
///
/// Reads the ally's `presenceId` from [allyControllerProvider] and the
  ...
  Future<void> sendMessage(String text) async {
    ...
    try {
      ...
      _subscription = stream.listen(
        ...
        onDone: () {
          if (!ref.mounted) return;
          // If we're still streaming when done fires (no done event received),
          // finalize with whatever we've accumulated.
