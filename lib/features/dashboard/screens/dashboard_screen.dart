...
Widget _buildLeftPanel(BuildContext context, int activeTab) {
  ...
  return switch (activeTab) {
    0 => const FieldStack(),
    1 => const GameScreen(),
    // Standings is where a player looks for where they place, so the
    // leaderboard lives here rather than buried inside the game modes.
    2 => Consumer(
      builder: (context, ref, _) => MedievalPokerLeaderboardScreen(
        viewerUserId: ref.watch(authControllerProvider).user?.id,
      ),
    ),
    _ => _ComingSoonPanel(
