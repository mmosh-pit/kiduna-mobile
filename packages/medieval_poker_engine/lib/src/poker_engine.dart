...
class PokerPlayer {
  ...
  /// 1-based order in which this player busted (1 = first out), or null while
  /// they still have chips. Final stacks alone can't rank the losers — everyone
  /// eliminated ends on 0 — so this is what lets results be ranked properly.
...
class PokerGame {
  ...
  /// Players busted so far, used to stamp [PokerPlayer.eliminationOrder].
  int _eliminationsSoFar = 0;
  ...
  void _finishHand(List<PotAward> awards) {
    ...
    for (final p in players) {
      if (!p.eliminated && p.stack <= 0) {
        p.eliminated = true;
        p.folded = true;
        p.eliminationOrder = ++_eliminationsSoFar;
