...
class _LiveRoom {
  ...
  /// Seats at this table. Per-room rather than server-wide so a tournament's
  /// heads-up final is a real 2-seat table instead of 2 humans + 2 AI.
  final int seatCount;
  ...
  _LiveRoom(this.id, this.game, this.agents,
      {required this.config, required this.seatCount, this.startThreshold = 1});
...
class GameServer {
  ...
  Future<HttpServer> start({String host = '0.0.0.0', int port = 8080}) async {
    ...
    server.listen((req) async {
      ...
      if (req.uri.path == '/game' && WebSocketTransformer.isUpgradeRequest(req)) {
        ...
        final seatsHint = int.tryParse(q['seats'] ?? '');
  ...
  _LiveRoom _ensureRoom(String roomId,
      {bool? timedOverride, int? seatsOverride}) {
    return _rooms.putIfAbsent(roomId, () {
      ...
      // Medieval Poker seats 2–4; anything outside that is clamped rather than
      // rejected so a malformed hint degrades to the server default.
      final seats = (seatsOverride ?? seatsPerRoom).clamp(2, seatsPerRoom);
      final players = [
      ...
      final agents = <PlayerAgent>[
        for (var i = 0; i < seats; i++) AiAgent(i, brain: AiBrain(rng: _rng)),
      ];
      final live = _LiveRoom(roomId, game, agents,
          config: cfg, seatCount: seats, startThreshold: humansToStart);
      live.room = GameRoom(
        roomId: roomId,
        game: game,
        agents: agents,
        aiSeats: {for (var i = 0; i < seats; i++) i},
  ...
  void _onConnect(String roomId, int seat, WebSocket socket,
    ...
    final live =
        _ensureRoom(roomId, timedOverride: timedHint, seatsOverride: seatsHint);
